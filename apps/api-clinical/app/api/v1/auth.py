...
async def require_actor(
    ...
    if not settings.CLINICAL_SUPABASE_URL or not settings.CLINICAL_SUPABASE_SECRET_KEY:
        raise HTTPException(
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as client:
            response = await client.get(
                f"{settings.CLINICAL_SUPABASE_URL.rstrip('/')}/auth/v1/user",
                headers={
                    "Authorization": f"Bearer {credentials.credentials}",
                    "apikey": settings.CLINICAL_SUPABASE_SECRET_KEY,
                },
            )
