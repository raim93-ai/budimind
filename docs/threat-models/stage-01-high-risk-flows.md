...
| Hostinger/MySQL            | Cross-plane access, public DB, backup loss, credential reuse  | Separate DB/users/backups, exact IP/TLS, field encryption, PITR/restore and isolation tests           |
...
flowchart LR
  ...
  A -->|published profiles only| D[(Clinical MySQL)]
  O[Clinical operations] -->|OIDC + role + step-up| A
  A -->|credential decision + audit| D
  D -. no route .-> C[(Corporate MySQL)]
```
...
flowchart LR
  ...
  A -->|atomic hold/booking| D[(Clinical MySQL)]
